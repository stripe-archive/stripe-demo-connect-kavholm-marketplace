...
options.presets=[...options.presets,presetItem];}options.caller.isServer=isServer;options.plugins=options.plugins||[];if(!isServer&&isPageFile){const pageConfigPlugin=babel.createConfigItem([require('../../babel/plugins/next-page-config')],{type:'plugin'});options.plugins.push(pageConfigPlugin);}if(isServer&&source.indexOf('next/data')!==-1){const nextDataPlugin=babel.createConfigItem([require('../../babel/plugins/next-data'),{key:(0,_path.basename)(filename)+'-'+(0,_stringHash.default)(filename)}],{type:'plugin'});options.plugins.push(nextDataPlugin);}if(isModern){const nextPreset=options.presets.find(preset=>preset&&preset.value===nextBabelPreset)||{options:{}};const additionalPresets=options.presets.filter(preset=>preset!==nextPreset);const presetItemModern=babel.createConfigItem(nextBabelPresetModern(nextPreset.options),{type:'preset'});options.presets=[...additionalPresets,presetItemModern];}// If the file has `module.exports` we have to transpile commonjs because Babel adds `import` statements
// That break webpack, since webpack doesn't support combining commonjs and esmodules
if(!hasModern&&source.indexOf('module.exports')!==-1){options.plugins.push(applyCommonJs);}options.plugins.push([require.resolve('babel-plugin-transform-define'),{'typeof window':isServer?'undefined':'object'},'next-js-transform-define-instance']);// As next-server/lib has stateful modules we have to transpile commonjs
options.overrides=[...(options.overrides||[]),{test:[/next-server[\\/]dist[\\/]lib/,/next[\\/]dist[\\/]client/,/next[\\/]dist[\\/]pages/],plugins:[commonJsItem]}];return options;}};});